...
export function createTrees(scene) {
  ...
  const coreMat = new THREE.MeshStandardMaterial({ roughness: 1.0, metalness: 0.0 });
  ...
  const groups = [
    ...
    ...range(3).map(() => {
      ...
      return {
        ...
        crown: buildLeafCrown({ cy, rx, ry, clumps: 13, size: 1.8 }),
...
function buildTieredTree({ h, tiers, r0 }) {
  ...
  for (let i = 0; i < tiers; i++) {
    const k = i / (tiers - 1);
    const y = h * (0.42 + 0.58 * k);
    const r = r0 * (1 - 0.68 * k) * (0.9 + Math.random() * 0.2);
    const thick = 0.5 + r * 0.14;
    ...
    // flattened lumpy blob — the leafy mass of the tier
    ...
    blob.scale(r * (0.88 + Math.random() * 0.24), thick, r * (0.88 + Math.random() * 0.24));
    blob.translate(ox, y, oz);
    coreParts.push(blob);

    // dense leaf cards on the tier's sunlit top surface
    const nCards = Math.max(7, Math.round(r * 4.5));
    for (let c = 0; c < nCards; c++) {
      ...
      card.rotateX(-Math.PI / 2 + (Math.random() - 0.5) * 0.5);
      const a = Math.random() * Math.PI * 2;
      const rr = Math.sqrt(Math.random()) * r * 0.95;
      const px = ox + Math.cos(a) * rr;
      const pz = oz + Math.sin(a) * rr;
      const py = y + thick * (0.55 + Math.random() * 0.5) * (1 - (rr / r) * 0.55);
      card.translate(px, py, pz);
      // up-tilted normals so the layers read as sunlit shelves
    ...
    const nRim = Math.max(5, Math.round(r * 2.6));
